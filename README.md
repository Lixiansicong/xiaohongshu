...
<details>
<summary><b>9. 🆕 模拟人类浏览推荐页</b></summary>

模拟真实用户在小红书推荐页的浏览行为，包括滚动、点击、浏览和互动。
...
- 📊 详细统计：返回浏览时长、滚动次数、点击次数、互动次数等

**配置参数：**

- duration: 浏览时长（分钟），默认 10 分钟
- click_probability: 点击笔记概率（0-100），默认 30%
- interact_probability: 互动概率（0-100），默认 50%
- comments: 评论内容列表（可选）

**防检测策略：**

- 行为随机化：滚动方式、距离、时间都有随机性
- 真实用户参数：滚动段时长 0.6-2.5s，短暂停 0.2-1.2s，回滚概率 7-18%
- 停留时间：中位数 3-6s，符合真实浏览习惯
- 自然退出：ESC 键或点击遮罩关闭笔记，保留滚动位置
- 概率机制：模拟真实用户的选择性行为
- 复用现有功能：点赞/收藏/评论使用已验证的安全方法

**详细文档：**
- [浏览功能完整说明](./docs/BROWSE_FEATURE.md)
- [人类行为模拟优化（v2.0）](./docs/BEHAVIOR_OPTIMIZATION.md) 🆕
...
- `like_feed` - 点赞或取消点赞笔记（需要：feed_id, xsec_token, unlike可选）
- `favorite_feed` - 收藏或取消收藏笔记（需要：feed_id, xsec_token, unfavorite可选）
- `browse_recommendations` - 🆕 模拟人类浏览推荐页（可选：duration, click_probability, interact_probability, comments）
...
互动概率设置为 70%，
使用以下评论内容：
["看完感觉收获很多，马上就去试试", "这个角度真的很新颖", 
"说得太对了", "保存下来慢慢看", "这个方法确实挺管用的",
"原来还可以这样", "照片拍得真好看", "有点心动了"]
```

或更详细的配置：

```
使用 xiaohongshu-mcp 的浏览功能：
- 浏览 15 分钟
- 每轮滚动 5-10 次
- 点击概率 60%
- 互动概率 80%
